...
complete -c nika -n "__fish_nika_needs_command" -f -a "evidence" -d 'Export the evidence pack for one run (journal + manifest + receipt + VERIFY.md)'
...
complete -c nika -n "__fish_nika_using_subcommand examples; and __fish_seen_subcommand_from run" -l var -d 'Set a workflow `inputs:` value (repeatable) — required examples name theirs in the header' -r
...
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -l color -d 'When to colour the output (auto = TTY + `TERM != dumb` · honours `CLICOLOR_FORCE` · `NO_COLOR` · `CLICOLOR=0` in that order)' -r -f -a "always\t'Force colour on (pagers accepting escapes · captured demos)'
never\t'Force colour off (the `--no-color` flags fold here)'
auto\t'Resolve from the environment chain + TTY (the default)'"
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -l hyperlink -d 'When to emit OSC-8 hyperlinks on printed paths (auto = TTY + `TERM != dumb` · never to pipes; always = force them, for pagers that pass escapes — tmux/screen may render them as plain text)' -r -f -a "always\t'Force hyperlinks on (escape-passing pagers · captured demos)'
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -l ascii -d 'Force the ASCII glyph twins everywhere (CI logs · legacy terminals) — colour stays; `--plain` is the full sober umbrella'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -l plain -d 'The sober umbrella — one flag for scripts, CI and transcripts: colour off · ASCII glyphs · hyperlinks off · no animation (`run` renders its plain storyboard). The same result as `--color never --hyperlink never` plus every verb\'s `--ascii`/`--no-progress`'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "replay" -d 'Re-render a run live (replay = re-render, NEVER re-execute)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "show" -d 'Print the final card only'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "ls" -d 'List the workspace trace store (`.nika/traces/`): age · size · workflow · terminal state (completed/failed/paused) · the resume-candidate marker (★ — the newest of each workflow, the trace retention never collects · ADR-100)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "rm" -d 'Remove traces from the store — one by name/path, `--older-than <dur>`, or `--all`. Removing a paused trace refuses without `--force` and names the unanswered prompt it would destroy (ADR-100)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "outputs" -d 'Browse per-task outputs: verb · duration · tokens · bounded preview (full value: `trace peek`)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "export" -d 'Project the journal to OTLP/JSON lines — every `OTel` tool becomes a viewer (drag into Jaeger UI ≥1.60 · POST lines to any OTLP/HTTP endpoint). Local file, zero collector, zero vendor'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "verify" -d 'Verify the journal\'s tamper-evidence chain (0.96+), then climb the proof ladder: SEALED (the `run_sealed` signature verifies against a custody key) · ANCHORED (the `<trace>.anchor.json` sidecar verifies fully offline) · REPLAYED (--replay compares a fresh run). The HIGHEST honestly-attained tier is reported. Exit 0 the tier holds · 2 broken/forged · 3 unchained (pre-chain journal) or a missing input'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "anchor" -d 'Notarize the journal head OUTSIDE the journal (S3): submit the post-seal head — signed with the run key — to the public Sigstore Rekor v2 transparency log plus an RFC 3161 timestamp, writing a detached `<trace>.anchor.json` sidecar. An explicit NETWORK act: this verb IS the opt-in. Exit 0 anchored · 2 the journal refuses (broken/torn) · 3 no key/network'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "reproduce" -d 'Is this run reproducible? Compare a recorded journal against a fresh one and classify every task: reproduced · nondeterministic (same def+inputs, different output) · authored · environment · status-changed · unverifiable. Exit 0 reproduced · 2 diverged'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "peek" -d 'Read ONE task\'s full output + its identity (hashes · duration · tokens). `--raw` prints the exact value only (pipe it to jq)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "flow" -d 'The data waterfall: which output fed which task, with recorded sizes (plan bindings from the workflow file × sizes from the trace)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay show ls rm outputs export verify anchor reproduce peek flow help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c nika -n "__fish_nika_using_subcommand evidence" -s o -l out -d 'Output directory (default: `<trace-stem>.evidence/` · never clobbered)' -r -F
complete -c nika -n "__fish_nika_using_subcommand evidence" -l workflow -d 'The workflow file that ran — hash-checked; unlocks the boundary, the trifecta verdict and the receipt' -r
complete -c nika -n "__fish_nika_using_subcommand evidence" -l color -d 'When to colour the output (auto = TTY + `TERM != dumb` · honours `CLICOLOR_FORCE` · `NO_COLOR` · `CLICOLOR=0` in that order)' -r -f -a "always\t'Force colour on (pagers accepting escapes · captured demos)'
never\t'Force colour off (the `--no-color` flags fold here)'
auto\t'Resolve from the environment chain + TTY (the default)'"
complete -c nika -n "__fish_nika_using_subcommand evidence" -l hyperlink -d 'When to emit OSC-8 hyperlinks on printed paths (auto = TTY + `TERM != dumb` · never to pipes; always = force them, for pagers that pass escapes — tmux/screen may render them as plain text)' -r -f -a "always\t'Force hyperlinks on (escape-passing pagers · captured demos)'
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand evidence" -l json -d 'Print the pack manifest to stdout (no directory written)'
complete -c nika -n "__fish_nika_using_subcommand evidence" -l ascii -d 'Force the ASCII glyph twins everywhere (CI logs · legacy terminals) — colour stays; `--plain` is the full sober umbrella'
complete -c nika -n "__fish_nika_using_subcommand evidence" -l plain -d 'The sober umbrella — one flag for scripts, CI and transcripts: colour off · ASCII glyphs · hyperlinks off · no animation (`run` renders its plain storyboard). The same result as `--color never --hyperlink never` plus every verb\'s `--ascii`/`--no-progress`'
complete -c nika -n "__fish_nika_using_subcommand evidence" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "welcome" -d 'The mirror: what Nika is · what this machine already has (editors · local models · key presence · this workspace) · the next commands. Offline · presence-only · always exit 0 — a greeting, not a gate'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "check" -d 'Audit a workflow BEFORE it runs: plan · cost ceiling · secret flows · types · tools — every finding teaches its fix'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "run" -d 'Run a workflow (the same audit runs first · live render)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "test" -d 'Golden test: run under the MOCK provider (offline · deterministic) and compare the typed `outputs:` against `<file>.golden.json`'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "inspect" -d 'Static anatomy: tasks · verbs · wave groups · cost · permits — and the ONE graph projector (`--format json|mermaid|dot` for the machine surfaces · human stays the default)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "explain" -d 'Teach one error code (cause · category · fix-form) — or narrate a workflow FILE: what it does · the waves · cost before a token is spent · what it touches · how to run it'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "key" -d 'The run-signing key lifecycle (mint · TOFU fingerprint · rotate — old pubs stay verifiable)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "sign" -d 'Sign a workflow file (S3 · author-binding): mint `<file>.minisig` · `--check` verifies'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "doctor" -d 'Diagnose this machine (binary · config · provider keys · local models). Diagnose-only — prints the exact fix command, never mutates anything'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "init" -d 'Found a repo (`.vscode` schema wiring · `AGENTS.md` · Cursor rule + MCP · `.agents/skills` authoring skill · optional workflow set). Bare on a terminal the founding wizard runs; flags are the scriptable twin. Existing files are skipped — `--force` overwrites'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "wire" -d 'Wire Nika into editor/agent MCP clients (explicit, idempotent)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "model" -d 'Local models — pull from the Hugging Face Hub, serve on this machine, list/rm the disk (ONE models dir · no external daemon)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "spec" -d 'The embedded spec identity (`--canon` prints the SSOT)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "catalog" -d 'The embedded provider/model catalog (models · capabilities · env vars)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "examples" -d 'Browse the embedded examples'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "new" -d 'Instantiate an embedded template skeleton'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "completions" -d 'Generate shell completions (bash · zsh · fish · elvish · powershell)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "trace" -d 'Read the flight recorder (replay or summarize a run)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "evidence" -d 'Export the evidence pack for one run (journal + manifest + receipt + VERIFY.md)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "dap" -d 'Debug Adapter Protocol server (stdio) — time-travel a recorded run under a debugger UI: breakpoints on task lines · step forward AND back through settles · outputs in the variables pane. Replay re-renders, never re-executes'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "lsp" -d 'Run the language server over stdio (drives the editor extension)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "mcp" -d 'Run the MCP server (validate: check/explain · learn: schema/examples/templates/canon — the in-binary Model Context Protocol surface for Cursor · Claude Desktop · agents). Default transport: stdio; `--transport http` serves Streamable HTTP for managed hosts. `approve` runs the CLIENT side: the MCP tool-pinning re-approval over the servers configured in `.nika/mcp_servers.json`'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog examples new completions trace evidence dap lsp mcp help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
