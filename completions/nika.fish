...
complete -c nika -n "__fish_nika_needs_command" -f -a "new" -d 'The ONE creation door: describe the job in plain words (routes), or name a slug/skeleton (takes it, ingredients included) — the destination derives from the slug. Bare `nika new` on a terminal is the guided flow; `nika new \'?\'` lists the skeleton set'
...
complete -c nika -n "__fish_nika_using_subcommand check" -l profile -d 'The readiness posture on the audit\'s risk grade (uncapped spend · glob/wildcard grants): `advisory` displays the grade on the verdict card, `operational` also fails (exit 2) when the grade is high or unbounded — the agent/CI readiness gate' -r -f -a "advisory\t'Grade displayed, never gating (the default)'
...
complete -c nika -n "__fish_nika_using_subcommand run" -l resume -d 'Resume from a prior run\'s NDJSON trace (`nika run … --json > trace.ndjson`): every task whose identity matches a journaled success is skipped with a visible `task_cache_hit` — an edited task or a changed input always re-runs (ADR-099). A trace without resume keys runs everything live (a notice, never an error). The trace\'s recorded engine version is JUDGED (F-P21): a resume under a different engine refuses, naming both versions' -r -F
...
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand try" -l all -d 'The whole shelf — the 13-step path plus every job (bare `nika try` shows three familiar jobs first)'
...
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -l ascii -d 'Force the ASCII glyph twins everywhere (CI logs · legacy terminals) — colour stays; `--plain` is the full sober umbrella'
...
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -f -a "replay" -d 'Re-render a run live (replay = re-render, NEVER re-execute)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -f -a "evidence" -d 'Export the evidence pack for one run (journal + manifest + receipt + VERIFY.md) — RAMS-15: one door on a run\'s dossier (read · export · prove), all under `trace`'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -f -a "receipt" -d 'Read a run receipt — `explain` renders its readable projection (stable text · a READING, never a proof)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -f -a "show" -d 'Print the final card only'
...
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -f -a "outputs" -d 'Browse per-task outputs: verb · duration · tokens · bounded preview (full value: `trace peek`)'
...
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -f -a "peek" -d 'Read ONE task\'s full output + its identity (hashes · duration · tokens). `--raw` prints the exact value only (pipe it to jq)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -f -a "flow" -d 'The data waterfall: which output fed which task, with recorded sizes (plan bindings from the workflow file × sizes from the trace)'
complete -c nika -n "__fish_nika_using_subcommand trace; and not __fish_seen_subcommand_from replay evidence receipt show ls rm outputs export verify anchor reproduce peek flow help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "welcome" -d 'The mirror: what Nika is · what this machine already has (editors · local models · key presence · this workspace) · the next commands. Offline · presence-only · always exit 0 — a greeting, not a gate'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "check" -d 'Audit a workflow BEFORE it runs: plan · cost ceiling · secret flows · types · tools — every finding teaches its fix'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "run" -d 'Run a workflow (the same audit runs first · live render)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "test" -d 'Golden test: run under the MOCK provider (offline · deterministic) and compare the typed `outputs:` against `<file>.golden.json`'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "inspect" -d 'Static anatomy: tasks · verbs · wave groups · cost · permits — and the ONE graph projector (`--format json|mermaid|dot` for the machine surfaces · human stays the default)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "explain" -d 'Teach one error code (cause · category · fix-form) — or narrate a workflow FILE: what it does · the waves · cost before a token is spent · what it touches · how to run it'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "key" -d 'The run-signing key lifecycle (mint · TOFU fingerprint · rotate — old pubs stay verifiable)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "sign" -d 'Sign a workflow file (S3 · author-binding): mint `<file>.minisig` · `--check` verifies'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "doctor" -d 'Diagnose this machine (binary · config · provider keys · local models). Diagnose-only — prints the exact fix command, never mutates anything'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "init" -d 'Found a repo (`.vscode` schema wiring · `AGENTS.md` · Cursor rule + MCP · `.agents/skills` authoring skill · optional workflow set). Bare on a terminal the founding wizard runs; flags are the scriptable twin. Existing files are skipped — `--force` overwrites'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "wire" -d 'Wire Nika into editor/agent MCP clients (explicit, idempotent). The door: `detected --dry-run` previews what this machine shows · `detected` wires it · `<client>` wires one · `all` is the advanced sweep (previewed, then confirmed or `--yes`)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "model" -d 'Local models — pull from the Hugging Face Hub, serve on this machine, list/rm the disk (ONE models dir · no external daemon)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "spec" -d 'The embedded spec identity (`--canon` prints the SSOT)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "catalog" -d 'The embedded provider/model catalog (models · capabilities · env vars)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "try" -d 'See a canonical workflow WORK — offline by default (the mock rehearsal · zero keys · zero flags), nothing written, nothing owned. Bare `nika try` lists what there is to see'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "new" -d 'The ONE creation door: describe the job in plain words (routes), or name a slug/skeleton (takes it, ingredients included) — the destination derives from the slug. Bare `nika new` on a terminal is the guided flow; `nika new \'?\'` lists the skeleton set'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "completions" -d 'Generate shell completions (bash · zsh · fish · elvish · powershell)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "trace" -d 'Read the flight recorder (replay or summarize a run)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "guard" -d 'The hook\'s judge (hidden — the wired `guard-run.sh` shim calls it, agents never type it): read a host hook payload (`--stdin`) or one command line (`--command`), find every effective `nika run`, audit the EXACT file in-process, and answer the hook protocol. P0-7 + P0-15: a red file or a priced model without `--max-cost-usd` is denied; an unjudgeable run is a VISIBLE `guard_unavailable`, never a silent allow. The run belongs to the human — guard JUDGES, it never executes'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "dap" -d 'Debug Adapter Protocol server (stdio) — time-travel a recorded run under a debugger UI: breakpoints on task lines · step forward AND back through settles · outputs in the variables pane. Replay re-renders, never re-executes'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "lsp" -d 'Run the language server over stdio (drives the editor extension)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "mcp" -d 'Run the MCP server (validate: check/explain · learn: schema/examples/templates/canon — the in-binary Model Context Protocol surface for Cursor · Claude Desktop · agents). Default transport: stdio; `--transport http` serves Streamable HTTP for managed hosts. `approve` runs the CLIENT side: the MCP tool-pinning re-approval over the servers configured in `.nika/mcp_servers.json`'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from welcome check run test inspect explain key sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
