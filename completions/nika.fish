...
complete -c nika -n "__fish_nika_needs_command" -f -a "explain" -d 'Teach one error code (cause · category · fix-form), a hint kind `nika check` printed in `[brackets]`, or narrate a workflow FILE: what it does · the waves · cost before a token is spent · what it touches · how to run it'
...
complete -c nika -n "__fish_nika_needs_command" -f -a "serve" -d 'Resident ARM firer by default (the SAME `fire`, wall clock in place of the OS). `--bind` + `--workflows` + `--token-file` opens authenticated loopback HTTP. Exit `0` clean · `1` otherwise'
...
complete -c nika -n "__fish_nika_needs_command" -f -a "init" -d 'Found a repo (`.vscode` schema wiring · `AGENTS.md` · Cursor rule + MCP · `.agents/skills` authoring skill · optional workflow set). Bare on a terminal the founding wizard runs; flags are the scriptable twin. Existing files are skipped (`.gitignore` is the one file updated in place: the traces line is added) — `--force` overwrites'
...
complete -c nika -n "__fish_nika_needs_command" -f -a "compile" -d 'Compile a skeleton, bounded support intent or conservative edit into a reviewable workflow. No destination: preview only; unknown intent stays incomplete'
...
complete -c nika -n "__fish_nika_needs_command" -f -a "mcp" -d 'Run the read-only MCP oracle (validate + learn — never execute). Default transport: stdio; `--transport http` serves Streamable HTTP for managed hosts. `approve` re-pins tools from `.nika/mcp_servers.json`'
...
complete -c nika -n "__fish_nika_using_subcommand check" -l access -d 'Judge the access plan under this pin (the same value `run --access` takes)' -r
...
complete -c nika -n "__fish_nika_using_subcommand check" -l sdk-snapshot -d 'Print the report with the execution snapshot the resident admits (the SDK\'s producer · the body `POST /v1/jobs` and `POST /v1/check` take · ADR-131)'
...
complete -c nika -n "__fish_nika_using_subcommand run" -l model -d 'Override only this workflow\'s default `model:` (`<provider>/<name>`). Task `model:` pins and invoked child workflows keep their own models. `mock/echo` does not make their calls or other effects offline. A bad id refuses when an infer/agent task resolves it' -r
complete -c nika -n "__fish_nika_using_subcommand run" -l access -d 'Pin the ACCESS path (`model:` picks the intelligence; access picks the path) — either a class (`local` · `mock` · `harness` · `oauth` · `api`) or a harness seat (`claude-code` · `codex` · `copilot` · `gemini-cli` · `grok-build` · `kimi-code` · `opencode` · `qwen-code`). Retired ACP wrapper ids (`claude-agent-acp` · `codex-acp`) refuse with NIKA-1802. A pin is a pin: unsatisfied refuses before the prologue with a witness, never substitutes another path or model (D-2026-08-04-N1). Example: `--model openai/gpt-5.2 --access codex` requests that model through the user\'s Codex subscription seat' -r
...
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand run" -l json -d 'Machine NDJSON: compact events or one pre-run refusal object, never pretty JSON'
complete -c nika -n "__fish_nika_using_subcommand run" -l cost-review-stdio -d 'Negotiate a fresh one-use local cost review over stdio; never approves a charge'
...
complete -c nika -n "__fish_nika_using_subcommand run" -l dry-run -d 'Plan only — show the static plan and execute ZERO effects (spec §10). This preview does not judge --max-cost-usd; the real run judges the budget. With `--json`: ONE versioned plan object (`plan_version: 1` — waves · cost ceiling · permits · requirements) instead of the human preview. `--output` stays refused (an outputs export of a run that never executed would be a lie)'
...
complete -c nika -n "__fish_nika_using_subcommand test" -l answer -d 'Answer a blocking `nika:prompt` without weakening it with a headless default (repeatable `TASK=VALUE`)' -r
complete -c nika -n "__fish_nika_using_subcommand test" -l var -d 'Bind an input for THIS case (repeatable `KEY=VALUE` · the same door as `run --var`); needs `--case` so the golden has a name' -r
complete -c nika -n "__fish_nika_using_subcommand test" -l case -d 'Name the case: the golden pins as `<file>.<NAME>.golden.json`, so one workflow carries a whole rule table (#1400)' -r
...
complete -c nika -n "__fish_nika_using_subcommand serve" -l bind -d 'Bind an authenticated HTTP listener. Requires `--workflows` and `--token-file`' -r
complete -c nika -n "__fish_nika_using_subcommand serve" -l workflows -d 'The served registry: the listener lists, schedules and ADMITS BY NAME (`POST /v1/jobs {"workflow": "<name>"}`) only the `.nika` workflows under this directory, named from the project root. A remote world rides the snapshot `nika check <file> --json --sdk-snapshot` prints. Requires `--bind`' -r -F
complete -c nika -n "__fish_nika_using_subcommand serve" -l token-file -d 'Owner-only Bearer file (32–512 visible ASCII bytes, mode 0600). Never argv. Mint: umask 077 && openssl rand -hex 24 > .nika/serve.token && chmod 600 .nika/serve.token' -r -F
complete -c nika -n "__fish_nika_using_subcommand serve" -l state-root -d 'Durable job-state root. Defaults to `<cwd>/.nika/serve`' -r -F
complete -c nika -n "__fish_nika_using_subcommand serve" -l authoring-model -d 'Seat native authoring on POST /v1/compile generation 2 with this direct provider model (`provider/name`); each caller still opts in (`cognition: "explicitProvider"`). Its key and endpoint are read from the environment now, never per request. Requires `--bind`' -r
complete -c nika -n "__fish_nika_using_subcommand serve" -l authoring-max-tokens -d 'Output tokens per call (1..=32768, default 8192); a caller may only narrow it' -r
complete -c nika -n "__fish_nika_using_subcommand serve" -l authoring-timeout -d 'Seconds per logical call (1..=600, default 120); the transport may resend one after a 429, 503 or 529 inside it, the compiler never repeats it' -r
complete -c nika -n "__fish_nika_using_subcommand serve" -l authoring-deadline -d 'Seconds per request (1..=3600, default 300): the work stops and the request answers 408' -r
complete -c nika -n "__fish_nika_using_subcommand serve" -l authoring-repairs -d 'Repair rounds per request (0..=5, default 3): at most 1 + N logical calls' -r
complete -c nika -n "__fish_nika_using_subcommand serve" -l knowledge -d 'A Foundry knowledge snapshot directory, verified and pinned at start; the seat reads the pack composed for each request\'s intent' -r -F
complete -c nika -n "__fish_nika_using_subcommand serve" -l knowledge-exclude -d 'A corpus whose examples the knowledge door never recalls' -r
...
complete -c nika -n "__fish_nika_using_subcommand serve" -l allow-remote -d 'Acknowledge a non-loopback `--bind`. Authentication is unchanged. TLS is a reverse proxy — this process does not terminate it'
...
complete -c nika -n "__fish_nika_using_subcommand init" -l theme -d 'Stamp the VS Code DAG canvas skin (`nika.dag.theme`) into the created `.vscode/settings.json` (`nika` the brand skin · `editor` adaptive · `phosphor` terminal green · `auto` lets the extension decide)' -r -f -a "nika\t''
editor\t''
phosphor\t''
auto\t''"
...
complete -c nika -n "__fish_nika_using_subcommand init" -l project-file -d 'Lay the starter `nika.yaml` — every scripted init does since #1283 (existing skipped · `--force` overrides); kept so older scripts work'
...
complete -c nika -n "__fish_nika_using_subcommand spec" -l schema -d 'Print the embedded JSON Schema for `*.nika` (the old `schema` verb, one roof)'
complete -c nika -n "__fish_nika_using_subcommand spec" -l project -d 'With `--schema`: the project file\'s schema (`nika.yaml`) instead of the workflow\'s'
...
complete -c nika -n "__fish_nika_using_subcommand try" -l resume -d 'Resume from a prior run\'s NDJSON trace — same flag, same parser as `nika run --resume`. Relative paths pin to the operator cwd (the rehearsal then enters a temp room)' -r -F
complete -c nika -n "__fish_nika_using_subcommand try" -l answer -d 'Answer a `nika:prompt` gate (repeatable · same parser as `nika run --answer`). Without `--resume` the answer is PRE-SEEDED on the fresh rehearsal (the CI one-pass gate)' -r
...
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand try" -l all -d 'The whole shelf — the numbered path plus every job (bare `nika try` shows the pack first-run shelf)'
...
complete -c nika -n "__fish_nika_using_subcommand compile" -s o -l output -d 'Explicit destination for edit mode (or create without a positional destination)' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l base -d 'Explicit accepted source for a conservative edit' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l change -d 'Supported edit: `Set const.NAME to JSON_LITERAL`' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l answer -d 'Answer a stable question: `KEY=JSON_LITERAL` (repeatable)' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l authoring-model -d 'Explicitly permit one provider call to interpret free intent (wire generation 2)' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l authoring-max-tokens -d 'Maximum authoring output tokens; requires explicit authoring model' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l authoring-timeout -d 'Timeout of each authoring call in seconds (the private plan\'s and every native call alike): 120 by default, 300 for a harness seat, at most 600; a call is never retried' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l hot-policy -d 'HOT admission contract: strict (default), legacy (pre-refactor, ablation) or off (never HOT for prose)' -r -f -a "strict\t''
legacy\t''
off\t''"
complete -c nika -n "__fish_nika_using_subcommand compile" -l authoring-samples -d 'Independent COLD proposals to compare (1..=5); each is one call. Requires the authoring model' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l authoring-strategy -d 'When the seat writes the candidate itself (a native `.nika` judged by the parser, the Check and the fidelity laws): `escalate` (default) after the private plan fails a human, `only` straight away, `sketch` (structure first: the seat sketches tasks, edges and gates, then fills typed holes, the compiler emits the file and derives the permits), `off` never. Requires the authoring model' -r -f -a "escalate\t''
only\t''
sketch\t''
off\t''"
complete -c nika -n "__fish_nika_using_subcommand compile" -l authoring-repairs -d 'Repair rounds a native candidate may buy from the compiler\'s diagnostics (0..=5, default 3)' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l knowledge -d 'A knowledge snapshot directory (manifest.json · one JSONL per kind · relations.jsonl): the seat reads the pack composed for this intent beside the card; the provenance names the snapshot. `NIKA_KNOWLEDGE` in the environment names one when the flag is absent' -r -F
complete -c nika -n "__fish_nika_using_subcommand compile" -l knowledge-exclude -d 'A corpus whose examples the knowledge door never recalls (a benchmark\'s own), for the snapshot `--knowledge` or `NIKA_KNOWLEDGE` names; refused when neither names one. `NIKA_KNOWLEDGE_EXCLUDE` in the environment names one when the flag is absent' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l knowledge-pack -d 'A pack another builder composed for THIS intent (JSON: `identity` · `selection` · `references: [{kind, id, text}]` · `repairs: {code: [strategy]}`): it enters the door as composed, identity and selection recorded verbatim, and wins over `--knowledge`. `NIKA_KNOWLEDGE_PACK` in the environment names one when the flag is absent' -r -F
complete -c nika -n "__fish_nika_using_subcommand compile" -l decision-model -d 'Explicitly seat one bounded-decision capability (`typesafe/jev-1.13.0` or `provider/name`) for finite ambiguities' -r
complete -c nika -n "__fish_nika_using_subcommand compile" -l color -d 'When to colour the output (auto = TTY + `TERM != dumb` · honours `CLICOLOR_FORCE` · `NO_COLOR` · `CLICOLOR=0` in that order)' -r -f -a "always\t'Force colour on (pagers accepting escapes · captured demos)'
never\t'Force colour off (the `--no-color` flags fold here)'
auto\t'Resolve from the environment chain + TTY (the default)'"
complete -c nika -n "__fish_nika_using_subcommand compile" -l hyperlink -d 'When to emit OSC-8 hyperlinks on printed paths (auto = TTY + `TERM != dumb` · never to pipes; always = force them, for pagers that pass escapes — tmux/screen may render them as plain text)' -r -f -a "always\t'Force hyperlinks on (escape-passing pagers · captured demos)'
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand compile" -l force -d 'Replace the explicitly named destination'
complete -c nika -n "__fish_nika_using_subcommand compile" -l fresh -d 'Ignore the plan recorded for this intent (`.nika/compile/<sha256>.plan.json`) and read or sample it again. An answer round otherwise replays that plan: zero provider calls'
complete -c nika -n "__fish_nika_using_subcommand compile" -l json -d 'Print the versioned structured result, including incomplete questions'
complete -c nika -n "__fish_nika_using_subcommand compile" -l list -d 'List exact skeleton names without authoring or writing'
complete -c nika -n "__fish_nika_using_subcommand compile" -l ascii -d 'Force the ASCII glyph twins everywhere (CI logs · legacy terminals) — colour stays; `--plain` is the full sober umbrella'
complete -c nika -n "__fish_nika_using_subcommand compile" -l plain -d 'The sober umbrella — one flag for scripts, CI and transcripts: colour off · ASCII glyphs · hyperlinks off · no animation (`run` renders its plain storyboard). The same result as `--color never --hyperlink never` plus every verb\'s `--ascii`/`--no-progress`'
complete -c nika -n "__fish_nika_using_subcommand compile" -s h -l help -d 'Print help (see more with \'--help\')'
...
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand trace; and __fish_seen_subcommand_from ls" -l json -d 'One JSON document (`traces: [{name, path, workflow, state, paused_task, bytes, modified, newest}]`) instead of the table'
...
never\t'Force hyperlinks off'
auto\t'TTY + `TERM != dumb` — never to pipes (the default)'"
complete -c nika -n "__fish_nika_using_subcommand trace; and __fish_seen_subcommand_from outputs" -l json -d 'One JSON document (`trace` · `tasks: [{id, verb, status, cause, error_code, recovered_from, …}]`) instead of the table'
...
complete -c nika -n "__fish_nika_using_subcommand trace; and __fish_seen_subcommand_from verify" -l sealed -d 'Require the seal tier: an UNSEALED journal (no `run_sealed` line) is exit 3 (a forged seal is exit 2 either way)'
complete -c nika -n "__fish_nika_using_subcommand trace; and __fish_seen_subcommand_from verify" -l json -d 'One JSON document instead of the prose ladder (`verify_version` 1 · `tier` · `exit` · `chain` · `seal` · `anchor` · `replay` · `lines`) — one document per line when several traces are named'
...
complete -c nika -n "__fish_nika_using_subcommand lsp" -l clientProcessId -d 'Same convention family: hosts pass their own PID so a server can watchdog its parent — and this one does. A host that closes the pipe ends the session by EOF; a host that CRASHES does not, and the server would outlive it forever (#1181). Hidden: no human types it, the editor does' -r
...
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "list" -d 'List the workflows below this directory, one relative path per line'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "welcome" -d 'The mirror: what Nika is · what this machine already has (editors · local models · key presence · this workspace) · the next commands. Offline · presence-only · always exit 0 — a greeting, not a gate'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "check" -d 'Audit a workflow BEFORE it runs: plan · cost ceiling · secret flows · types · tools — every finding teaches its fix'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "run" -d 'Run a workflow (the same audit runs first · live render)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "test" -d 'Golden test: run under the MOCK provider (offline · deterministic) and compare the typed `outputs:` against `<file>.golden.json`'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "inspect" -d 'Static anatomy: tasks · verbs · wave groups · cost · permits — and the ONE graph projector (`--format json|mermaid|dot` for the machine surfaces · human stays the default)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "explain" -d 'Teach one error code (cause · category · fix-form), a hint kind `nika check` printed in `[brackets]`, or narrate a workflow FILE: what it does · the waves · cost before a token is spent · what it touches · how to run it'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "key" -d 'The run-signing key lifecycle (mint · TOFU fingerprint · rotate — old pubs stay verifiable)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "arm" -d 'What this project has ARMED, and when each beat next fires. Read-only — it schedules nothing (the file proposes, the machine disposes). Exit `0` clean · `2` the registry refuses'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "serve" -d 'Resident ARM firer by default (the SAME `fire`, wall clock in place of the OS). `--bind` + `--workflows` + `--token-file` opens authenticated loopback HTTP. Exit `0` clean · `1` otherwise'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "sign" -d 'Sign a workflow file (S3 · author-binding): mint `<file>.minisig` · `--check` verifies'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "doctor" -d 'Diagnose this machine (binary · config · provider keys · local models). Diagnose-only — prints the exact fix command, never mutates anything'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "init" -d 'Found a repo (`.vscode` schema wiring · `AGENTS.md` · Cursor rule + MCP · `.agents/skills` authoring skill · optional workflow set). Bare on a terminal the founding wizard runs; flags are the scriptable twin. Existing files are skipped (`.gitignore` is the one file updated in place: the traces line is added) — `--force` overwrites'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "wire" -d 'Wire Nika into editor/agent MCP clients (explicit, idempotent). The door: `detected --dry-run` previews what this machine shows · `detected` wires it · `<client>` wires one · `all` is the advanced sweep (previewed, then confirmed or `--yes`)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "model" -d 'Local models — pull from the Hugging Face Hub, serve on this machine, list/rm the disk (ONE models dir · no external daemon)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "spec" -d 'The embedded spec identity (`--canon` prints the SSOT)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "catalog" -d 'The embedded provider/model catalog (models · capabilities · env vars)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "try" -d 'See a canonical workflow WORK — offline by default (the mock rehearsal · zero keys · zero flags), nothing written, nothing owned. Bare `nika try` lists what there is to see'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "compile" -d 'Compile a skeleton, bounded support intent or conservative edit into a reviewable workflow. No destination: preview only; unknown intent stays incomplete'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "completions" -d 'Generate shell completions (bash · zsh · fish · elvish · powershell)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "trace" -d 'Read the flight recorder (replay or summarize a run)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "guard" -d 'The hook\'s judge (hidden — the wired `guard-run.sh` shim calls it, agents never type it): read a host hook payload (`--stdin`) or one command line (`--command`), find every effective `nika run`, audit the EXACT file in-process, and answer the hook protocol. P0-7 + P0-15: a red file or a priced model without `--max-cost-usd` is denied; an unjudgeable run is a VISIBLE `guard_unavailable`, never a silent allow. The run belongs to the human — guard JUDGES, it never executes'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "dap" -d 'Debug Adapter Protocol server (stdio) — time-travel a recorded run under a debugger UI: breakpoints on task lines · step forward AND back through settles · outputs in the variables pane. Replay re-renders, never re-executes'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "lsp" -d 'Run the language server over stdio (drives the editor extension)'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "mcp" -d 'Run the read-only MCP oracle (validate + learn — never execute). Default transport: stdio; `--transport http` serves Streamable HTTP for managed hosts. `approve` re-pins tools from `.nika/mcp_servers.json`'
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
