...
complete -c nika -n "__fish_nika_using_subcommand run" -l model -d 'Override the workflow\'s envelope `model:` only (`<provider>/<name>`). Per-task `model:` pins stay live and metered — `--model` does not descend into them (B22). Resolved through the SAME path as an envelope model — a bad id fails loud when an infer/agent task resolves it. `--model mock/echo` rehearses the envelope seat offline (zero key · zero network)' -r
...
complete -c nika -n "__fish_nika_using_subcommand serve" -l workflows -d 'The served registry: the listener lists, schedules and ADMITS BY NAME (`POST /v1/jobs {"workflow": "<name>"}`) only the `.nika.yaml` workflows under this directory, named from the project root. A remote world rides the snapshot `nika check <file> --json --sdk-snapshot` prints. Requires `--bind`' -r -F
