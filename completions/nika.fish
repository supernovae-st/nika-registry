...
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "explain" -d 'Teach one error code (cause · category · fix-form), a hint kind `nika check` printed in `[brackets]`, or narrate a workflow FILE: what it does · the waves · cost before a token is spent · what it touches · how to run it'
...
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "serve" -d 'Resident ARM firer by default (the SAME `fire`, wall clock in place of the OS). `--bind` + `--workflows` + `--token-file` opens authenticated loopback HTTP. Exit `0` clean · `1` otherwise'
...
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try new completions trace guard dap lsp mcp help" -f -a "mcp" -d 'Run the read-only MCP oracle (validate + learn — never execute). Default transport: stdio; `--transport http` serves Streamable HTTP for managed hosts. `approve` re-pins tools from `.nika/mcp_servers.json`'
