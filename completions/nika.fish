...
complete -c nika -n "__fish_nika_needs_command" -f -a "compile" -d 'Compile an exact skeleton or conservative edit into a reviewable workflow. No destination: preview only; unknown intent stays incomplete'
...
complete -c nika -n "__fish_nika_using_subcommand compile" -l output -d 'Explicit destination for edit mode (or create without a positional destination)' -r
...
complete -c nika -n "__fish_nika_using_subcommand help; and not __fish_seen_subcommand_from list welcome check run test inspect explain key arm serve sign doctor init wire model spec catalog try compile completions trace guard dap lsp mcp help" -f -a "compile" -d 'Compile an exact skeleton or conservative edit into a reviewable workflow. No destination: preview only; unknown intent stays incomplete'
