...
complete -c nika -n "__fish_nika_using_subcommand check" -l sdk-snapshot -d 'Internal SDK adapter: emit the report with its immutable snapshot'
...
complete -c nika -n "__fish_nika_using_subcommand serve" -l workflows -d 'Held registry root of `.nika.yaml` workflows. Requires `--bind`' -r -F
